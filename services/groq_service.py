...
"""
    ...
    @staticmethod
    def extract_questions_from_text(pdf_text, subject, difficulty_level, api_key):
        """
...
4. The difficulty level of the question. Use the default difficulty '{difficulty_level}' (or classify it as 'easy', 'medium', 'complex', or 'very_complex' based on the question's depth).
5. The subject of the question. Use the default subject '{subject}' (or override it if a more specific subject is clear from the text).
...
"""
        ...
        try:
            ...
            for idx, item in enumerate(questions_list):
                ...
                diff = item.get("difficulty", difficulty_level).lower()
                if diff not in ['easy', 'medium', 'complex', 'very_complex']:
